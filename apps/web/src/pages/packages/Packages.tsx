...
import { Icon, useMagneticHover, useSpotlight } from '@atlas-south/design-system';
...
import { PlanFinder } from '../../components/packages/PlanFinder.js';
 ...
 */
/**
 * One pricing tier card — its own component (not inlined in the `.tiers.map()` below) so
 * `useMagneticHover`/`useSpotlight` each get a genuine per-card hook instance; calling a
 * hook conditionally inside a loop body breaks React's rules of hooks.
 */
function TierCard({ tier }: { tier: PricingTier }) {
  ...
  return (
    <div
      ref={spotlightRef}
      className={`spotlight-card relative flex flex-col rounded-lg border p-6 ${
        tier.popular ? 'border-accent-blue bg-accent-blue/5 shadow-lg' : 'border-border bg-canvas'
      }`}
    >
        ...
        <ul className="mt-4 space-y-2">
          {tier.includes.map((item) => (
            <li key={item} className="flex items-start gap-2 text-sm text-slate">
              <Icon name="badge-check" size={16} className="mt-0.5 flex-shrink-0 text-accent-blue" />
              {item}
            </li>
          ))}
        </ul>
      ...
      {tier.excludes && tier.excludes.length > 0 && (
        <ul className="mt-2 space-y-2">
          {tier.excludes.map((item) => (
            <li key={item} className="flex items-start gap-2 text-sm text-slate/50">
      ...
      */}
      {import.meta.env.VITE_PAYPAL_CLIENT_ID && tier.paypalPlanId ? (
        ...
        <Link
          ...
          className={`mt-6 flex min-h-[44px] items-center justify-center rounded-lg px-4 text-sm font-semibold transition-transform duration-150 ease-out ${
            tier.popular
...
export function Packages() {
  ...
  const bottomCta = useMagneticHover<HTMLAnchorElement>();
  ...
  return (
    <>
      ...
      {/* Plan finder — a shortcut for undecided visitors above the full comparison
          below, not a replacement for it. Uses the same tier data (real, restored
          boundaries), never a separate copy of the pricing logic. */}
      <section className="pb-16 sm:pb-20">
        <div className="mx-auto max-w-4xl px-4">
          <PlanFinder tiers={data.tiers} />
        </div>
      </section>
      ...
      <section className="bg-canvas-tint py-16 sm:py-20">
        <div className="mx-auto max-w-4xl px-4 text-center">
          ...
          <Link
            ...
            className="mt-6 inline-block rounded-lg bg-accent-blue px-8 py-3 font-semibold text-white transition-transform duration-150 ease-out hover:bg-brand-blue"
          >
            Get a quote
          </Link>
        </div>
