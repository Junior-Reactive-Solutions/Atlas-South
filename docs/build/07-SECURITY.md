...
| `Content-Security-Policy` | `default-src 'self'; img-src 'self' res.cloudinary.com; script-src 'self'; style-src 'self' fonts.googleapis.com; font-src fonts.gstatic.com; connect-src 'self' <api-origin>; frame-ancestors 'none'` | Audit: currently absent entirely. Locked to only the origins this project actually uses. |
